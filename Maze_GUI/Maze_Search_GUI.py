'''This Project that helps to visualize the behavior of the 
        Depth First Search, Breadth First Search, and A* Algorithms.'''


import Base
import Configuration
import Search_Algorithms
import turtle
import tkinter as tk
from playsound import playsound

class Maze_Search_GUI:

    def __init__(self, maze_grid, maze_dimensions, maze_obstacles, bot_start_pos) -> None:
        self.maze_grid = maze_grid
        self.maze_dimensions = maze_dimensions
        self.maze_obstacles = maze_obstacles
        self.bot_start_pos = self.bot_pos = bot_start_pos
        self.bot_path = None
        self.doing_path = False
        self.show_path = True
        self.treasure_pos = (0, 0)
        self.treasure_set = False
        self.path_started = False
        self.algorithm = 'dfs'

        # Turtle Window
        self.screen = turtle.Screen()
        self.screen.setup(Configuration.WIDTH, Configuration.HEIGHT)
        self.screen.title('Maze Search Algorithms')
        self.canvas = self.screen.getcanvas()

        # prepare game and start game loop.
        self.init_screen()
        self.reset()

    def init_screen(self):
        # Create screen and controls.
        self.screen.bgcolor('white')
        # Disable animation to update the screen manually.
        self.screen.tracer(0)
        self.create_controls()
        self.screen.listen()

        # Builder.
        self.builder = turtle.Turtle()
        self.builder.shape('square')
        self.builder.color('red')
        self.builder.shapesize(18 / 20)
        self.builder.hideturtle()
        self.builder.penup()

        # bot.
        self.bot = turtle.Turtle()
        self.bot.shape('circle')
        self.bot.color('black')
        self.bot.shapesize(18 / 20)
        self.bot.penup()

        # Treasure.
        self.treasure = turtle.Turtle()
        self.treasure.shape('circle')
        self.treasure.color('green')
        self.treasure.shapesize(18 / 20)
        self.treasure.penup()

        # Score turtle.
        self.score_turtle = turtle.Turtle()
        self.score_turtle.hideturtle()
        self.score_turtle.color('purple')
        self.score_turtle.penup()
        self.score_turtle.goto(0, -350)

        # Draw maze objects and walls.
        self.draw_maze(self.maze_obstacles)

    def reset(self):
        # Stop any movement and clear the trail.
        self.bot.clearstamps()
        self.bot_path = None
        self.stop_doing_path()

        # Position/show game objects.
        self.bot_pos = self.bot_start_pos
        self.treasure.hideturtle()
        self.treasure_set = False
        self.draw_piece(self.bot, self.bot_start_pos)

        # Start game.
        self.path_started = False
        # Some events may have been unbound to prevent event stacking.
        self.bind_events()
        self.screen.update()
        self.animation_loop()

    def set_treasure_pos(self, x, y):
        if not self.path_started:           # Not allowed if path has started.
            self.treasure_pos = Base.screen_to_grid_coords(
                (x, y), self.maze_dimensions)
            if self.treasure_pos not in self.maze_obstacles:
                self.treasure.goto(Base.grid_to_screen_coords(
                    self.treasure_pos, self.maze_dimensions))
                self.treasure.showturtle()
                self.treasure_set = True
                self.screen.update()

    def bind_events(self):
        self.screen.onkey(self.do_algorithm, 's')
        self.screen.onkey(self.toggle_doing_path, 'space')
        self.screen.onclick(self.set_treasure_pos)

    def create_controls(self):
        self.reset_button = tk.Button(
            self.canvas.master,
            width=5,
            text='Reset',
            bg='blue',
            fg='white',
            bd=5,
            activebackground='green',
            activeforeground='white',
            font=Configuration.BUTTON_FONT,
            command=lambda: self.reset(),
        )
        self.path_button = tk.Button(
            self.canvas.master,
            width=5,
            text='Path',
            bg='blue',
            fg='red',
            bd=5,
            activebackground='blue',
            activeforeground='white',
            font=Configuration.BUTTON_FONT,
            command=self.toggle_show_path,
        )

        self.dfs_button = tk.Button(
            self.canvas.master,
            width=5,
            text='DFS',
            bg='blue',
            fg='red',
            bd=5,
            activebackground='blue',
            activeforeground='white',
            font=Configuration.BUTTON_FONT,
            command=lambda: self.update_algorithm('dfs'),
        )

        self.bfs_button = tk.Button(
            self.canvas.master,
            width=5,
            text='BFS',
            bg='blue',
            fg='white',
            bd=5,
            activebackground='blue',
            activeforeground='white',
            font=Configuration.BUTTON_FONT,
            command=lambda: self.update_algorithm('bfs'),
        )

        self.a_star_button = tk.Button(
            self.canvas.master,
            width=5,
            text='A*',
            bg='blue',
            fg='white',
            bd=5,
            activebackground='blue',
            activeforeground='white',
            font=Configuration.BUTTON_FONT,
            command=lambda: self.update_algorithm('a_star'),
        )

        self.canvas.create_window(-500, 120, window=self.reset_button)
        self.canvas.create_window(-500, 60, window=self.path_button)
        self.canvas.create_window(-500, 0, window=self.dfs_button)
        self.canvas.create_window(-500, -60, window=self.bfs_button)
        self.canvas.create_window(-500, -120, window=self.a_star_button)

    def toggle_show_path(self):
        self.show_path = not self.show_path
        if self.show_path:
            self.path_button.config(fg='red')
        else:
            self.path_button.config(fg='white')

    def update_algorithm(self, alg):
        self.algorithm = alg
        buttons = [self.dfs_button, self.bfs_button, self.a_star_button]
        for button in buttons:
            button.config(fg='white')
        if alg == 'dfs':
            self.dfs_button.config(fg='red')
        elif alg == 'bfs':
            self.bfs_button.config(fg='red')
        elif alg == 'a_star':
            self.a_star_button.config(fg='red')
        self.reset()

    def draw_maze(self, obstacles):
        for obstacle in obstacles:
            self.builder.goto(Base.grid_to_screen_coords(
                obstacle, self.maze_dimensions))
            self.builder.stamp()

    def draw_piece(self, piece: turtle.Turtle, pos, stamp=False):
        piece.goto(Base.grid_to_screen_coords(pos, self.maze_dimensions))
        if stamp:
            piece.stamp()

    def toggle_doing_path(self):
        if self.path_started:
            if self.doing_path:
                self.stop_doing_path()
            else:
                self.start_or_continue_path()

    def do_algorithm(self):
        if self.treasure_set:  # Can't find path to null position.
            if self.algorithm == 'dfs':
                self.bot_path = Search_Algorithms.dfs(self.maze_grid, self.bot_start_pos, 
                                                           self.treasure_pos)
            elif self.algorithm == 'bfs':
                self.bot_path = Search_Algorithms.bfs(self.maze_grid, self.bot_start_pos, 
                                                           self.treasure_pos)
            elif self.algorithm == 'a_star':
                self.bot_path = Search_Algorithms.a_star(self.maze_grid, self.bot_start_pos, 
                                                              self.treasure_pos)
            #Path found.
            if self.bot_path is not None:
                self.path_started = True
                self.screen.onkey(None, 's')
                self.start_or_continue_path()
            else:
                self.reset()   #Goal unreachable.

    def start_or_continue_path(self):
        self.doing_path = True
        self.animation_loop()
    
    def stop_doing_path(self):
        self.doing_path = False
    
    def animation_loop(self):
        if self.doing_path:
            try:
                next_pos = self.bot_path.pop(0)
            except IndexError:
                return 
            self.draw_piece(self.bot, next_pos, self.show_path)
            if next_pos == self.treasure_pos:
                playsound('sounds/treasure_found.mp3', False)
            self.screen.update()
            turtle.ontimer(self.animation_loop, Configuration.GAME_SPEED)

if __name__ == '__main__':
    maze_grid, maze_dimensions, maze_obstacles, bot_start_pos = Base.read_maze(
        Configuration.MAZE_FILE)
    Maze_Search_GUI(maze_grid, maze_dimensions, maze_obstacles, bot_start_pos)
    turtle.done()